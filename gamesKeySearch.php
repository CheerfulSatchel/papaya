<html>
  ...
  $( document ).ready(function() {
    ...
    $.ajax({
      type: "POST",
      url: 'readGamesKeyword.php',
      dataType: 'json',
      data: {key:keyword},
      success: function (response) {
        var trHTML = "";
        $.each(response, function (i, val) {
            trHTML += "<tr><td>" + val.title + "</td><td>" + val.platform + "</td><td>" + val.genre + "</td><td>" + val.publisher + "</td><td>" + val.rating +
						 "</td><td>" + val.price + "</td><td><button type='button' class='btn-md' onClick=\"(buySong('" + val.item_id + "," + email + "'))\">Add To Cart</button></td>";
        });
  ...
  <title>VideoGames Section</title>

  </head>

	<h1> VideoGames Section </h1>
	<br />
