...
<?php
  ...
  //$email = $_POST['userName'];
 	$stmt = $db->prepare("SELECT * FROM sell NATURAL JOIN item NATURAL JOIN movie NATURAL JOIN user");
  //$stmt->bind_param("s", $email);
  $stmt->execute();
  $MovieResult = $stmt->get_result();

  $stmt = $db->prepare("SELECT * FROM sell NATURAL JOIN item NATURAL JOIN video_game NATURAL JOIN user");
  //$stmt->bind_param("s", $email);
  $stmt->execute();
  $VideoGameResult = $stmt->get_result();

  $stmt = $db->prepare("SELECT * FROM sell NATURAL JOIN item NATURAL JOIN song NATURAL JOIN user");
  //$stmt->bind_param("s", $email);
  $stmt->execute();
